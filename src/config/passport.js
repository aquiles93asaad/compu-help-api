...
const localLogin = new LocalStrategy({
    ...
    let user = await User.findOne({ email }, 'name lastName email hashedPassword image esAdmin favouriteComputers');
    if (!user) {
...
const jwtLogin = new JwtStrategy({
    ...
    let user = await User.findById(payload._id, 'name lastName email image esAdmin favouriteComputers');
